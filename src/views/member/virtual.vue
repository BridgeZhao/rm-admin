<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button @click="getInStoreData(scope.row)">
            进店记录
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-row v-if="!showType" v-loading="tableLoading">
      <el-col v-for="(item, index) in tableData" :key="index" :xs="8" :sm="4" :md="5" :lg="4" :xl="3">
        <div class="user-box">
          <p class="avatar">
            <img :src="item.avatar">
          </p>
          <p style="text-align: center">
            <el-tag type="info">
              {{ storeList.find(s_item => { return s_item.id === item.storeId }).name }}
            </el-tag>
          </p>
          ...
          <div class="flex space-between mg-top10">
            ...
            <div class="col-1 text-right">
              <label>性别：</label><span>{{ item.gender==='m'?'男':item.gender==='w'?'女':'未知' }}</span>
            </div>
          </div>
          <p><span>{{ item.createTime | dateformat('YYYY-MM-DD HH:mm:ss') }}</span></p>
          <p class="button">
            <el-button @click="getInStoreData(item)">
              进店记录
            </el-button>
          </p>
        </div>
    ...
    <el-dialog v-if="dialogVisible" v-drag-dialog title="进店记录" :width="'720px'" :visible.sync="dialogVisible">
      <el-row :gutter="20" class="dg-head">
        ...
        <el-col :span="16">
          ...
          <p class="inline">
            <span>进店时间：{{ editData.createTime | dateformat('YYYY-MM-DD HH:mm:ss') }}</span>
          </p>
          <ul class="dg-info">
            <li>
              <span>累计进店</span>
