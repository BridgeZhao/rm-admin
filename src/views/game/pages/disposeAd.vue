  ...
  <div class="dispose-ad">
    ...
    <el-dialog v-drag-dialog :title="dialogType==='add'?'创建广告':dialogType === 'detail'?'广告详情':'广告编辑'" append-to-body
      ...
      <el-form v-loading="loading" :model="form" :rules="rules" ref="myform">
        ...
        <el-row v-else :gutter="24" id="imgs-dev">
          <el-col :span="11" class="">
            ...
            <el-form-item label="投放时间" prop="formTime" :label-width="formLabelWidth">
              <el-date-picker v-model="form.formTime" type="daterange" range-separator="至" start-placeholder="开始时间"
                end-placeholder="结束时间" size="mini" style="width:100%;" :picker-options="pickerOptions"></el-date-picker>
            </el-form-item>
    ...
    <el-row :gutter="20" class="table-head-btns">
      ...
      <el-col :span="8" class="flex">
        <el-input style="width: 300px;margin-right: 10px" placeholder="请输入名称" v-model="pagination.name">
    ...
    <el-table v-loading="loading" :data="tableData" border stripe style="width: 100%">
      ...
      <el-table-column label="操作" width="260">
        <template slot-scope="scope">
          <div>
            <el-button v-permission="'delete'" type="danger" size="small" @click.native.prevent="cancelRow(scope.row.id)"
              v-if="(scope.row.status === 0 || scope.row.status === 1)">
              撤销
            </el-button>
            <el-button size="small" @click.native.prevent="editRow(scope.row)" v-if="(scope.row.status === 0 || scope.row.status === 1)">
              编辑
            </el-button>
    ...
    <nav class="nav-page">
      <el-pagination :page-size="pagination.size" layout="prev, pager, next, jumper" :total="pagination.total"
